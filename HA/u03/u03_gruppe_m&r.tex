...
\item   $T(n) = T\left(\frac{9}{10}n\right) + n$\\
        ...
        \begin{align*}
         ...
         &=\frac{9}{10} \cdot n \leq 1 \cdot \frac{9}{10} \cdot f(n)
        \end{align*}
        Fall 3: $T(n) = \Theta(n)$
...
\item   $T(n) = T(\sqrt{n}) + 1$\\
        MT nicht anwendbar\\
        Bestimme die Anzalh der Rekursionsschritte $x$, bis $n \leq 2$:
        \begin{align*}
         n^{\frac{1}{2^x}} &\leq 2 \tag*{$|\ \log_n$}\\
         \Leftrightarrow \log_n 2 &\leq \frac{2}{2^x} \tag*{$| \cdot -1 \log_2$} \\
